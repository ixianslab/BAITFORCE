"""
monai_transform_trainv5_2_0.py
...
ROI_LOCAL = (128,128,128)
ROI_GLOBAL = (192,192,192)
...
# Konfiguration
# ─────────────────────────────────────────────
BASE_DIR_UNLABELED  = "./workspace/data/MMDental"
#LABELS_JSON         = "D:/chewing_gum/DATASETS/labels.json"
CACHE_DIR_GLOBAL    = "./workspace/cache_global_monai"
CACHE_DIR_LOCAL     = "./workspace/cache_local_monai"
CHECKPOINT_SSL      = "./workspace/checkpoint_ssl"
#CHECKPOINT_REG      = "./checkpoint_regression.pt"
MODEL_SSL_FINAL     = "./workspace/SSL_checkpoints/ssl_model_final.pt"
#MODEL_REG_FINAL     = "./regression_model_final.pt"
MONITOR_DIR         = "./workspace/monitoring"       # Collapse-Monitoring Plots
...
# SSL Training
SSL_BATCH_SIZE      = 8
ACCUMULATION_STEPS  = 2        # effektive Batch Size = 16
NUM_WORKERS         = 16
...
class HybridMAEEncoder3D(nn.Module):
    ...
    """
    def __init__(self, embed_dim: int = 768, patch_size: int = MAE_PATCH_SIZE):
        super().__init__()
...
class MAEDecoder3D(nn.Module):
    ...
    """
    def __init__(self, embed_dim: int = 768, decoder_dim: int = 128,
...
class MultiScaleSSLModel(nn.Module):
    def __init__(self, global_dim=1024, local_dim=512, proj_dim=512):
        super().__init__()
...
def run_ssl_training(ssl_mode: str = "mae"):
    ...
    if ssl_mode == "mae":
        mae_model = MAEModel(embed_dim=768, decoder_dim=128).to(device)
        ...
        for epoch in range(start_epoch, SSL_EPOCHS):
            ...
            if epoch % CHECKPOINT_INTERVAL == 0:
                ckpt_path = f"{CHECKPOINT_SSL}_epoch_{epoch:03d}.pt"
                save_checkpoint(epoch, mae_model, optimizer, avg_loss, ckpt_path)
        ...
        for epoch in range(start_epoch, SSL_EPOCHS):
            ...
            if epoch % CHECKPOINT_INTERVAL == 0:
                ckpt_path = f"{CHECKPOINT_SSL}_epoch_{epoch:03d}.pt"
                save_checkpoint(epoch, ms_model, optimizer, avg_loss, ckpt_path)
...
def run_regression_training(ssl_mode: str = "mae"):
    ...
    if ssl_mode == "mae":
        ssl_model = MAEModel(embed_dim=768).to(device)
    ...
    for epoch in range(start, REG_EPOCHS):
        ...
        if epoch % CHECKPOINT_INTERVAL == 0:
            ckpt_path = f"{CHECKPOINT_REG}_epoch_{epoch:03d}.pt"
            save_checkpoint(epoch, regressor, optimizer, avg, ckpt_path)
