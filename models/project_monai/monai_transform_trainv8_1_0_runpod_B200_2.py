"""
monai_transform_trainv8_1_0_runpod_h100.py
...
# ─────────────────────────────────────────────
# ÄNDERUNGEN FÜR H100 80GB / 230GB RAM
# Kommentare zeigen Original → Neu
# ─────────────────────────────────────────────

# H100-spezifische Flags — ganz oben ins Skript einfügen
import torch
torch.backends.cuda.enable_flash_sdp(True)      # Flash Attention für Transformer
torch.backends.cudnn.benchmark = True            # cudnn auto-tuning
torch.set_float32_matmul_precision("high")       # TF32 für matmul
...
# Konfiguration
# ─────────────────────────────────────────────
BASE_DIR_UNLABELED  = "./workspace/workspace"
...
# Multi-Scale ROI
ROI_GLOBAL = (192, 192, 192)   # ganzer Schaedel resized — Morphologie
...
MONITOR_INTERVAL    = 10       # [8] Collapse-Check alle N Epochs

# SSL Training
SSL_BATCH_SIZE      = 16
ACCUMULATION_STEPS  = 1        # effektive Batch Size = 16
NUM_WORKERS         = 16
LR_SSL              = 2e-4   # [3] leicht hoeher als 1e-4
BYOL_TAU            = 0.996
...
def mandible_focused_cropd(data):
    img = data["image"]

    C, D, H, W = img.shape
    mandible_start_h = int(H * 0.40)

    roi_d, roi_h, roi_w = (96, 96, 96)

    d_start = torch.randint(0, max(1, D - roi_d), (1,)).item()
    h_start = torch.randint(
        mandible_start_h,
        max(mandible_start_h + 1, H - roi_h),
        (1,)
    ).item()
...
def create_views_global(batch: dict):
    ...
    for i in range(x.shape[0]):
        x1_list.append(aug_global({"image": x[i].clone()})["image"])
        x2_list.append(aug_global({"image": x[i].clone()})["image"])
...
def create_views_local(batch: dict):
    ...
    x1_list, x2_list = [], []
    for i in range(x.shape[0]):
        # Zwei unabhängige Crops: tensor.clone() statt deepcopy
        c1 = rand_crop({"image": x[i].clone()})
        c2 = rand_crop({"image": x[i].clone()})
...
class HybridMAEEncoder3D(nn.Module):
    ...
    """
    def __init__(self, embed_dim: int = 1536, patch_size: int = MAE_PATCH_SIZE):
        super().__init__()
...
def _block_masking_3d(B, D, H, W, mask_ratio, device):
    """
    Vektorisierte Version — kein Python for-Loop.
    Chebyshev-Distanz via Broadcasting → läuft auf GPU.
    ~50-100x schneller als Python-Loops bei 1728 Patches.
    """
    N = D * H * W
    num_masked = int(N * mask_ratio)
    r = max(1, int((num_masked * 3 / (4 * 3.14159)) ** (1/3)))

    # Zufällige Mittelpunkte [B, 1, 3]
    centers = torch.stack([
        torch.randint(0, D, (B,), device=device),
        torch.randint(0, H, (B,), device=device),
        torch.randint(0, W, (B,), device=device),
    ], dim=1).float().unsqueeze(1)

    # Grid [1, N, 3]
    dg, hg, wg = torch.meshgrid(
        torch.arange(D, device=device),
        torch.arange(H, device=device),
        torch.arange(W, device=device),
        indexing="ij",
    )
    grid = torch.stack([dg, hg, wg], dim=-1).reshape(1, N, 3).float()

    # Chebyshev-Distanz [B, N] — kein Python-Loop
    dists = (grid - centers).abs().max(dim=-1).values
    mask  = dists <= r

    # Randkorrektur falls zu wenige maskiert
    for b in range(B):
        n_masked = mask[b].sum().item()
        if n_masked < num_masked:
            unmasked = torch.where(~mask[b])[0]
            n_extra  = min(num_masked - n_masked, len(unmasked))
            if n_extra > 0:
                idx = unmasked[torch.randperm(len(unmasked), device=device)[:n_extra]]
                mask[b, idx] = True
...
class MAEDecoder3D(nn.Module):
    ...
    """
    def __init__(self, embed_dim: int = 1536, decoder_dim: int = 256,
...
def run_ssl_training(ssl_mode: str = "mae"):
    ...
    local_loader = DataLoader(
        ...
        prefetch_factor=4,       # war nicht gesetzt → Prefetch für H100
        drop_last=True,
    )
    # NEU — kein Scaler, kein Accumulation:
    #scaler = torch.amp.GradScaler(device="cuda")

    if ssl_mode == "mae":
        mae_model = MAEModel(embed_dim=1536, decoder_dim=256).to(device)
        # Nach: mae_model = MAEModel(embed_dim=512, decoder_dim=128).to(device)
        mae_model = torch.compile(mae_model, mode="reduce-overhead")
        # Erste Epoch dauert länger (JIT-Kompilierung) — danach deutlich schneller
        ...
        for epoch in range(start_epoch, SSL_EPOCHS):
            ...
            for step, local_batch in enumerate(local_loader):
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
                    loss, _ = mae_model(x, mask_ratio=MAE_MASK_RATIO, use_block_masking=True)
            ...
            if torch.isnan(torch.tensor(avg_loss)):
                ...
                save_checkpoint(epoch, mae_model, optimizer, avg_loss, f"{CHECKPOINT_SSL}_{epoch}.pt")
                return
        ...
        for epoch in range(start_epoch, SSL_EPOCHS):
            ...
            if torch.isnan(torch.tensor(avg_loss)):
                print("  [Fehler] NaN — Training abgebrochen.")
                save_checkpoint(epoch, ms_model, optimizer, avg_loss, f"{CHECKPOINT_SSL}_{epoch}.pt")
                return
...
def predict_kaukraft(scan_path: str, ssl_mode: str = "mae") -> float:
    ...
    if ssl_mode == "mae":
        ssl_model = MAEModel(embed_dim=1536).to(device)
